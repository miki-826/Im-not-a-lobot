...
function parseMetrics(v: unknown): VoiceMetrics | undefined {
  ...
  const percent = (x: unknown) => clampNumber(x, 0, 100);
  return {
    avgVolume: percent(m.avgVolume),
    peakVolume: percent(m.peakVolume),
    silenceRatio: percent(m.silenceRatio),
    firstSpeechDelayMs: clampNumber(m.firstSpeechDelayMs, -1, 60_000, -1),
    volumeVariation: percent(m.volumeVariation),
    shout: !!m.shout,
    spoke: !!m.spoke,
  };
}
...
export async function POST(req: Request) {
  ...
  const texts: string[] = Array.isArray(body?.texts)
    ? body.texts.slice(0, 3).map((t: unknown) => safeString(t, 500))
    : [];
  ...
  const input = {
    ...
    hasMic: !!body?.hasMic,
    completedTaskCount: Math.round(clampNumber(body?.completedTaskCount, 0, 3)),
    voiceMetrics,
  };
