...
export async function POST(req: Request) {
  ...
  const taskId = safeString(body?.taskId, 40) || "task-1";
  const taskTitle = safeString(body?.taskTitle, 120);
  const instruction = safeString(body?.instruction, 500);
  const userText = safeString(body?.userText, 500);
