...
このゲームは**顔画像そのものをAIで解析しているわけではありません**。判定は次のシグナルを
組み合わせた「人間らしさ」評価です（演出として審査ログを流していますが、内部処理は以下のとおり）。

- **テキスト**：愚痴・生活感・言い淀みなどのキーワードと文量
- **音声特徴量**：ブラウザの Web Audio API で測った声の揺れ・最大音量・沈黙率・話し始めの遅延・叫び判定（録音はしない）
- **行動シグナル**：カメラ提出の有無、マイク使用の有無、回答の有無・完了数

> 発表時は「顔のAI解析」ではなく「**カメラ提出・音声・テキスト・行動ログを組み合わせた人間らしさ判定**」と説明してください。

音声認識（話した内容のテキスト化）は `SpeechRecognition / webkitSpeechRecognition` を使うため
**Chrome系で最も安定**します。非対応ブラウザでも、テキスト入力で最後まで進められます。
